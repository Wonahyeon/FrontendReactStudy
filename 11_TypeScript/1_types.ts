...
// 1) 첫번째 방법: type[]
let firstArray: number[] = [1, 2, 3]; // 숫자만 가능한 배열

// 2) 두번째 방법(제네릭): Array<type>
let secondArray: Array<number> = [4, 5, 6]; // 숫자만 가능한 배열
...
let person2: {name: string, age: number}  = { name: 'wonah', age: 25 };
// 이 변수에는 객체만 들어올 수 있는데 name은 string, age는 number 타입만 가능

let person3: {name: string, age: number, job?: string}  = { name: 'wonah', age: 25 };
// ? 사용 시 job 속성은 옵셔널
...
const todayWeek: Week = Week.SUN; // 7
...
let unionValue3: {name: string | number} = {name: 'won'};
